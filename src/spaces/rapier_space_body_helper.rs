...
impl RapierSpace {
    ...
    pub fn rapier_intersect_aabb(
        ...
        let count = physics_engine.intersect_aabb(
        ...
        // B-SORT (determinism prototype): the broad phase yields colliders in BVH-traversal order,
        // which is regenerated differently after import_state (godot-rapier #534). The kinematic
        // motion recovery (recover_motion_from_contacts — order-dependent accumulation) and the
        // cast tie-break (cast_motion — strict-< first-wins) both consume this order, so a shuffled
        // order flips the resolved contact on replay. Sort by the stable Godot collider identity
        // (UserData{part1,part2}, set at collider creation, independent of Rapier handle order) so
        // the iteration order is identical in record and replay. Pure reordering of an existing
        // result set — no collision math is changed. Only the 3 motion-path callers reach here.
        if count > 1 {
            results[0..count as usize].sort_by_key(|r| (r.user_data.part1, r.user_data.part2));
        }
        count
    }
